...
class Github {
    static async commits(_data) {
        ...
        const res = await axios.get(GITHUB_AUTHTHENTIFICATION_CONSTANTS.COMMITS_URL(repository), {
            headers: {
        ...
        instance.setVariable('$1$', res.data[0].commit.message);
        instance.setVariable('$2$', res.data[0].commit.author.name);
        instance.setVariable('$3$', res.data[0].commit.author.date);
